...
import { newBreakpointMap } from "../theme/base";
...
const mediaQueries: MediaQueries = (() => {
  ...
  return Object.keys(newBreakpointMap).reduce((accum, size, index) => {
    // Largest size is just a min-width of second highest max-width
    if (index === Object.keys(newBreakpointMap).length - 1) {
      return { ...accum, [size]: `(min-width: ${prevMinWidth}px)` };
    }

    const minWidth = prevMinWidth;
    const breakpoint = newBreakpointMap[size];
