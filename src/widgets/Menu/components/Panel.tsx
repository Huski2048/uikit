import React from "react";
import styled from "styled-components";
import PanelBody from "./PanelBody";
import PanelFooter from "./PanelFooter";
import { SIDEBAR_WIDTH_FULL, SIDEBAR_WIDTH_REDUCED } from "../config";
import { PanelProps, PushedProps } from "../types";
import Logo from "./Logo";

interface Props extends PanelProps, PushedProps {
  showMenu: boolean;
  isMobile: boolean;
  togglePush: () => void;
}

const StyledPanel = styled.aside<{ isPushed: boolean; showMenu: boolean }>`
  position: fixed;
  // padding-top: 20px;
  padding: ${({ isPushed }) => (isPushed ? "0px 18px 0 " : "0")};
  top: 0;
  left: 0;
  display: flex;
  flex-direction: column;
  justify-content: space-between;
  flex-shrink: 0;
  background-color: ${({ theme }) => theme.nav.background};
  width: ${({ isPushed }) => (isPushed ? `260px` : 0)};
  height: 100%;
  transition: padding-top 0.2s, width 0.2s cubic-bezier(0.4, 0, 0.2, 1);
  border-right: ${({ isPushed }) =>
    isPushed ? "2px solid rgba(133, 133, 133, 0.1)" : 0};
  z-index: 11;
  overflow: ${({ isPushed }) => (isPushed ? "initial" : "hidden")};
  transform: translate3d(0, 0, 0);
  ${({ isPushed }) => !isPushed && "white-space: nowrap;"};
  * {
    font-family: "GenJyuuGothic" !important;
  }
  ${({ theme }) => theme.mediaQueries.nav} {
    font-size: 12px;
  }
  ${({ theme }) => theme.mediaQueries.nav} {
    border-right: unset;
    width: ${({ isPushed }) =>
      `${isPushed ? SIDEBAR_WIDTH_FULL : SIDEBAR_WIDTH_REDUCED}px`};
  }
`;

const Panel: React.FC<Props> = (props) => {
  const { isPushed, showMenu, links, isDark } = props;
  const homeLink = links.find((link) => link.label === "Home");
  return (
    <StyledPanel isPushed={isPushed} showMenu={showMenu}>
      <div>
        <Logo
          isPushed={isPushed}
          href={homeLink?.href ?? "/"}
          isDark={isDark}
          togglePush={props.togglePush}
        />
        <PanelBody {...props} />
      </div>
      <PanelFooter {...props} />
    </StyledPanel>
  );
};

export default Panel;
